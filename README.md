...
Le dépôt cloné est uniquement le générateur. Aucun projet étudiant n’existe tant qu’une destination distincte ne contient pas le manifeste de génération et que les gates enregistrées ne sont pas vertes. Le premier JSON fournit directement les cinq questions de validation et interdit toute déclaration prématurée de projet prêt.
...
Chaque projet généré expose `pnpm setup`, `pnpm run doctor`, `pnpm migrate`, `pnpm dev`, `pnpm test`, `pnpm build` et `pnpm smoke:auth` lorsque l’identité est disponible. Consultez [le démarrage rapide](docs/getting-started/quickstart.md) et [les profils](docs/architecture/profiles.md).
